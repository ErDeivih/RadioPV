...
describe('adapt Â· TrackOut â†’ la forma que espera la interfaz', () => {
  ...
  it('el Ã¡lbum siempre lleva una imagen (aunque falte la carÃ¡tula)', () => {
  ...
  it('no revienta con los campos opcionales vacÃ­os', () => {
...
describe('adapt Â· listas', () => {
  const lista = { id: 7, name: 'Mi lista', description: '', type: 'user', n_tracks: 3, user_id: 5 } as any;

  it('el dueÃ±o es el de verdad, para poder saber si la lista es tuya', () => {
    expect(toPlaylist(lista).owner.id).toBe('5');
    // Las listas que genera la aplicaciÃ³n no tienen dueÃ±o: no son de nadie.
  ...
  it('la portada propia se pide pasando por la API (si no, la imagen sale rota)', () => {
    ...
    // la API: en producciÃ³n, detrÃ¡s de `/api`. Sin ese prefijo, nginx contesta el index.html de la
    // aplicaciÃ³n (con un 200), la imagen no carga y el anÃ¡lisis de color se queda sin hacer.
    // Se compara con API_BASE porque en las pruebas apunta a otro sitio que en producciÃ³n
...
describe('adapt Â· usuario', () => {
  it('conserva is_admin (sin esto el panel da 403 a todo el mundo)', () => {
    // La API manda `is_admin` en /auth/me, pero el adaptador no lo copiaba: la interfaz creÃ­a que
    // nadie era administrador y el panel de administraciÃ³n â€”con los atajos de limpieza, el borrado
    // en masa y la lista negraâ€” contestaba 403 Â«solo para administradoresÂ» incluso al dueÃ±o.
    const admin = toUser({ id: 1, email: 'a@b.c', display_name: 'A', is_admin: true } as any);
    expect(admin.is_admin).toBe(true);

    const normal = toUser({ id: 2, email: 'c@d.e', display_name: 'C', is_admin: false } as any);
    expect(normal.is_admin).toBe(false);

    // Un usuario sin el campo (respuesta antigua) no debe quedar como administrador.
    expect(toUser({ id: 3, email: 'e@f.g', display_name: 'E' } as any).is_admin).toBe(false);
  });
});

describe('ganancia por canciÃ³n', () => {
